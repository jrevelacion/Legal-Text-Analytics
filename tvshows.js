 const apiKey = 'd1209528ce772d0348c92edfc5fdfb73'; // Your TMDB API Key
    ...
    const SERIES_ENDPOINTS = [
        { url: 'https://vidfast.pro/tv/', name: 'Epic(Fast, Auto Next, Auto Play)' },
        { url: 'https://111movies.com/tv/', name: 'Mythic(Fast, Auto Next, Auto Play)' },
        { url: 'https://player.videasy.net/tv/', name: 'Mythical(Fast, Auto Next, Auto Play)' },
    ...
    const fetchTVShowDetails = async () => {
        try {
            ...
            const tvShowFirstAirDate = tvShow.first_air_date; // Dynamically fetched air date
            ...
            // Fetch TV Show Cast (Leading Actors)
            const castUrl = `${baseUrl}/tv/${tvShowId}/credits?api_key=${apiKey}&language=en-US`;
            const castResponse = await fetch(castUrl);
            const castData = await castResponse.json();
        
            const castContainer = document.getElementById('movie-cast-container'); // You may want to rename this to "tv-show-cast-container"
        
            // Loop through cast and append actors
            castData.cast.slice(0, 9).forEach(actor => { // Limit to 8 actors
                const actorElement = document.createElement('div');
                actorElement.classList.add('actor');
            
                const actorImageUrl = actor.profile_path ? `https://image.tmdb.org/t/p/w500${actor.profile_path}` : 'https://via.placeholder.com/100x150?text=No+Image';
                const actorImage = document.createElement('img');
                actorImage.src = actorImageUrl;
    ...
    changeServerBtn.addEventListener('click', () => {
        ...
        SERIES_ENDPOINTS.forEach((server, index) => {
            ...
            serverItem.addEventListener('click', () => {
                ...
                else if (server.name === 'Mythical(Fast, Auto Next, Auto Play)') {
                    iframe.src = `${server.url}${tvShowId}/${selectedSeason}/${selectedEpisode}?nextEpisode=true&autoplayNextEpisode=true&episodeSelector=true&color=8B5CF6`; // Add custom Warrior parameters
                } 
                else if (server.name === 'Epic(Fast, Auto Next, Auto Play)') {
                    iframe.src = `${server.url}${tvShowId}/${selectedSeason}/${selectedEpisode}?autoPlay=true`; // Add custom Warrior parameters
                }    
                else {
...
// Function to toggle the chat popup (open/close)
function toggleChat() {
    var popup = document.getElementById("popupChat");

    // If the popup is hidden, show it with animation
    if (popup.classList.contains("show")) {
        closeChat();  // Close the chatbox if it's open
    } else {
